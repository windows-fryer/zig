...
pub const ReturnCode = enum(usize) {
    _,

    pub fn fromSigned(x: isize) ReturnCode {
        return @bitCast(x);
    }

    pub fn fromUnsigned(x: usize) ReturnCode {
        return @enumFromInt(x);
    }

    pub fn toSigned(rc: ReturnCode) isize {
        return @bitCast(@intFromEnum(rc));
    }

    pub fn toUnsigned(rc: ReturnCode) usize {
        return @intFromEnum(rc);
    }

    pub fn toFd(rc: ReturnCode) fd_t {
        return @intCast(@intFromEnum(rc));
    }

    pub fn errno(rc: ReturnCode) E {
        const signed = rc.toSigned();
...
pub const E = switch (native_arch) {
    ...
    .sparc, .sparcel, .sparc64 => enum(u16) {
        ...
        pub fn init(rc: usize) E {
            return ReturnCode.fromUnsigned(rc).errno();
        }
    },
};

pub const abi = @import("linux/abi.zig");

pub const pid_t = i32;
pub const fd_t = enum(i32) { _ };
